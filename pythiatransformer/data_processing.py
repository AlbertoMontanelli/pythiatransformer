...
def train_val_test_split(tensor, train_perc=0.6, val_perc=0.2, test_perc=0.2):
    ...
    if not (train_perc + val_perc + test_perc == 1):
        raise ValueError("Invalid values for split percentages. Must sum to 1.")
...
# Dict ID & One-hot
eos_token = -999
id_all = np.unique(np.concatenate([
    ak.flatten(data_23["id_23"]),
    ak.flatten(data_final["id_final"])
]))
dict_ids = {int(pid): idx for idx, pid in enumerate(id_all)}
dict_ids[eos_token] = len(dict_ids)
num_classes = len(dict_ids)

one_hot_23 = one_hot_encoding(padded_tensor_23, dict_ids, num_classes, eos_token)
one_hot_final = one_hot_encoding(padded_tensor_final, dict_ids, num_classes, eos_token)
...
if __name__ == "__main__":
    ...
    assert torch.all(last_ids == eos_index), "❌ Alcuni target batchati non terminano con EOS!"
